...
@section('content')
    <div class="container-fluid py-4" style="color:#e0e0e0;">
        ...
        <div class="card bg-dark border-0 shadow-sm rounded-4 mb-4" style="overflow:hidden;">
            <div class="card-header text-dark fw-bold border-0" style="background:linear-gradient(90deg,#ffc107,#d67400);">
            ...
            <div class="card-body text-light">
                ...
                <p><strong class="text-warning">Data de Emissão:</strong>
                    {{ \Carbon\Carbon::parse($pedido->created_at)->format('d/m/Y') }}</p>

                @if ($pedido->frete)
                    <p><strong class="text-warning">Valor do Frete:</strong> R$
                        {{ number_format($pedido->frete->valor_frete, 2, ',', '.') }}</p>
                @else
                ...
                @php
                    ...
                    $badgeClass = match (strtolower($status)) {
                        ...
                        'coleta realizada' => 'bg-success text-light',
                        'coleta não realizada' => 'bg-danger text-light',
                        'aguardando transferência' => 'bg-secondary text-light',
                        'em processo de transferência' => 'bg-warning text-dark',
                        'transferência realizada' => 'bg-success text-light',
                        'transferência não realizada' => 'bg-danger text-light',
                        'em processo de separação no destino' => 'bg-info text-dark',
                        'em rota de entrega' => 'bg-primary text-light',
                        'entrega realizada' => 'bg-success text-light',
                        'entrega não realizada' => 'bg-danger text-light',
                ...
                <p class="mb-0">
                    ...
                    <strong class="text-warning">Status Atual:  </strong>
        ...
        @if ($pedido->rotas->isNotEmpty())
            <div class="card bg-dark border-0 shadow-sm rounded-4">
                <div class="card-header text-dark fw-bold border-0"
                    style="background:linear-gradient(90deg,#ffc107,#d67400);">
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-center">
                                            <button class="btn btn-outline-info btn-sm fw-semibold"
                                                data-bs-toggle="collapse" data-bs-target="#historico{{ $rota->id_rotas }}">
                                            ...
                                            <table class="table table-sm table-dark table-striped rounded-3 mb-0">
                                                ...
                                                <tbody>
                                                    @foreach ($rota->historicos as $movimentacao)
                                                        ...
                                                        @include('pedido.modais.modal_comprovante', [
